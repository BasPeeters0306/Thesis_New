...
def RF_test(best_model, window_size, y_in_sample, X_in_sample, y_test, X_test):
    ...
    Parameters
    ----------
    best_model : sklearn.ensemble._forest.RandomForestClassifier
    ...
    model = RandomForestClassifier(n_estimators = int(best_model["n_estimators"].iloc[0]), 
    ...
    progress_bar = tqdm(total=int(np.ceil(len(y_test)/window_size)), unit='iteration')         # Must bet tested!!!!!!!!!!!!
...
def RF_tune(X_train, y_train, X_val, y_val, grid):
    ...
    gridsearch_results = pd.DataFrame(columns= ['n_estimators','max_depth', "Accuracy"])
    # Loop over grid
    for i in tqdm(list(grid.values())[0]):
        for j in list(grid.values())[1]:
