...
def add_lags(df):
    ...
    for i in range(1, lags+1):
        df[f'AdjustedClose_Lag_{i}'] = df.groupby('SymbolExchangeCode')['AdjustedClose'].shift(i)
    
    # Create ... leads of "AdjustedClose" for each stock
    leads = 5
    for i in range(1, leads+1):
        df[f'AdjustedClose_Lead_{i}'] = df.groupby('SymbolExchangeCode')['AdjustedClose'].shift(-i)
...
def cap_outliers(df, str_column_name):
    ...
    if str_column_name == 'PreviousdayReturn':
        df = df[(df[str_column_name] < .8) & (df[str_column_name] > -0.8)]
...
def create_historical_SP_Index():
    ...
    df_temp['BarDate'] = pd.to_datetime(df_temp['BarDate'])
    df3_temp['BarDate'] = pd.to_datetime(df3_temp['BarDate'])

    # Resample and forward fill within each group of "SymbolExchangeCode"
    df3_daily_list = []
    for symbol in df3_temp['SymbolExchangeCode'].unique():
        df3_temp_symbol = df3_temp[df3_temp['SymbolExchangeCode'] == symbol]
        df3_temp_symbol.set_index('BarDate', inplace=True)  # move set_index operation inside the loop
        df3_daily_symbol = df3_temp_symbol.resample('D').ffill()
    ...
    # Merge df_temp and df3_daily, keeping only rows where 'SymbolExchangeCode' and 'BarDate' match
    df_final = pd.merge(df_temp, df3_daily, how='inner', on=['BarDate', 'SymbolExchangeCode'])
...
def add_SESI(df, df_SESI):
    ...
    df_SESI = df_SESI.drop(columns = ["TIMESTAMP_UTC"])
