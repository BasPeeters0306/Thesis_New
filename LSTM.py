...
from tensorflow.keras.layers import LSTM, Dense, Dropout, SimpleRNN, GRU         #Input, Concatenate         
...
def LSTM_test(best_model, window_size, y_in_sample, X_in_sample, y_test, X_test, b_sentiment_score, n_past_returns, loss_function, str_nn_type):
    ...
    model = LSTM_model_1(dropout = best_model["dropout"].iloc[0], 
                                       ...
                                       learning_rate = best_model["learning_rate"].iloc[0],
                                        ...
                                        sequence_length = best_model["sequence_length"].iloc[0], b_sentiment_score=b_sentiment_score, n_past_returns=n_past_returns, n_nodes = best_model["n_nodes"].iloc[0], loss_function=loss_function, str_nn_type=str_nn_type)
...
def LSTM_tune(X_train, y_train, X_val, y_val, grid, b_sentiment_score, n_past_returns, loss_function, str_nn_type):
    ...
    gridsearch_results = pd.DataFrame(columns=["n_nodes", "dropout", "recurrent_dropout", "learning_rate", "batch_size", "optimizer", "sequence_length", "val_loss", "epochs_min_val_loss", "val_accuracy", "epochs_min_acc_loss", "evaluation_dict"])         

    # Loop over grid
    for n_nodes in tqdm(list(grid.values())[0]):
        for dropout in tqdm(list(grid.values())[1]):
            for recurrent_dropout in tqdm(list(grid.values())[2]):
                for learning_rate in list(grid.values())[3]:
                    for batch_size in list(grid.values())[4]:
                        for optimizer in list(grid.values())[5]:
                            for sequence_length in list(grid.values())[6]:
                                    model = LSTM_model_1(dropout, recurrent_dropout, learning_rate, optimizer, sequence_length, b_sentiment_score, n_past_returns, n_nodes, loss_function, str_nn_type)
                                    ...
                                    gridsearch_results.loc[len(gridsearch_results)] = [n_nodes, dropout, recurrent_dropout, learning_rate, batch_size, optimizer, sequence_length, min(evaluation_dict["val_loss"]), 
...
def LSTM_model_1(dropout, recurrent_dropout, learning_rate, optimizer, sequence_length, b_sentiment_score, n_past_returns, n_nodes, loss_function, str_nn_type):
    # Function for custom_loss
    # Still test custom_loss (after regular thing works)
    def custom_loss(y_true, y_pred):
        # Compute the 10th and 90th percentiles
        y_pred_10 = tensorflow.stats.percentile(y_pred, 10, axis=0, keepdims=True)
        y_pred_90 = tensorflow.stats.percentile(y_pred, 90, axis=0, keepdims=True)
        # Create masks for the top and bottom deciles
        mask_bottom = tensorflow.cast(tensorflow.less_equal(y_pred, y_pred_10), dtype=tensorflow.float32)
        mask_top = tensorflow.cast(tensorflow.greater_equal(y_pred, y_pred_90), dtype=tensorflow.float32)
        # Combine the masks
        mask = mask_bottom + mask_top
        # Apply the masks to y_true and y_pred
        y_true_masked = y_true * mask
        y_pred_masked = y_pred * mask
        # Compute the binary cross entropy loss
        loss = BinaryCrossentropy(y_true_masked, y_pred_masked)
        return loss
    ...
    model = Sequential()
    # Add an LSTM layer with n_nodes hidden neurons and input shape (sequence_length, n_features)
    # sequence_length is the number of time steps and n_features is the number of features  
    print("n_nodes", n_nodes)
    print("str_nn_type", str_nn_type)
    print("sequence_length", sequence_length)
    print("n_features", n_features)
    print("dropout", dropout)
    print("recurrent_dropout", recurrent_dropout)
    if str_nn_type == "LSTM":
        model.add(LSTM(n_nodes, input_shape=(sequence_length, n_features), dropout=dropout, recurrent_dropout=recurrent_dropout))
    elif str_nn_type == "SimpleRNN":
        model.add(SimpleRNN(n_nodes, input_shape=(sequence_length, n_features), dropout=dropout, recurrent_dropout=recurrent_dropout))
    elif str_nn_type == "GRU":
        model.add(GRU(n_nodes, input_shape=(sequence_length, n_features), dropout=dropout, recurrent_dropout=recurrent_dropout))
...
# def LSTM_model_1(dropout, recurrent_dropout, learning_rate, optimizer, sequence_length, b_sentiment_score, n_past_returns, n_nodes):
#     if b_sentiment_score == True:
#         n_features = n_past_returns + 1
#     else:
#         n_features = n_past_returns
...
#     # sequence_length is the number of time steps and n_features is the number of features
#     model.add(Dense(units = n_nodes, input_shape=(sequence_length, n_features), activation="relu"))
#     model.add(LSTM(units = 32, dropout=dropout, recurrent_dropout=recurrent_dropout))#32?
#     # Add a Dense output layer with 1 neuron and sigmoid activation
#     model.add(Dense(1, activation="sigmoid"))
#     # Compile the model
#     if optimizer == "RMSprop":
